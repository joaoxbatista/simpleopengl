...
def draw():
	...
	print("Y = " + str(sphere_object.y)) 
	print("FLAG = " + str(direction)) 

	if(sphere_object.y >= -2 and sphere_object.y <= 4 and direction):
		sphere_object.addY()
	else:
		sphere_object.velocity = 0.04
		direction = False

	if(not direction):
		sphere_object.decY()

		if(sphere_object.y <= -2):
			sphere_object.y = -2
			sphere_object.velocity = 0.2
			direction = True
